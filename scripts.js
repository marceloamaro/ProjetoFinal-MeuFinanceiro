...
const Storage = {
  get() {
    return JSON.parse(localStorage.getItem('meufinanceiro:transaction')) || [];
  },
  set(transaction) {
    localStorage.setItem('meufinanceiro:transaction', JSON.stringify(transaction));
  }
}
...
const Transaction = {
  ...
  download(e) {
    var pdf = './arquivos/requisitos.pdf'
    e.preventDefault();
...
const Utils = {
  ...
  createPDF(e) {
    ...
    doc.text(' 2021 - Meu Financeiro$$', 105, 285, null, null, "center");

    doc.save(`meufinanceiro-${todayName}.pdf`);
  }
}
